...
@preconditions(lambda id : id is not None and len(id) > 0,
                lambda class_id : class_id is not None and len(class_id) > 0,
               lambda context_id : context_id is not None and len(context_id) > 0)
def gen_properties(id:str, class_id: str, context_id:str, description:str = '')->str:
    '''
    Generate the properties string for an ADLINK Edge SDK Thing.
    '''
    import json
    data = {}
    data['id'] = id
